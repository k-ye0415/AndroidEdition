...
Task 의 이전 `Activity` 는 destroy 되지 않고 **stopped 상태**를 유지한다. 사용자가 이전 `Activity` 로 돌아갈 경우를 대비하여 UI 상태가 유지된다.
...
- `Activity Instances` 는 재배열되지 않는다.  
  - `Back Stack` 의 `Activity` 는 절대 재배열되지 않는다. 앱에서 사용자가 `여러 Activity` 에서 `동일한 Activity` 를 시작할 수 있도록 허용하는 경우 해당 `Activity` 는 새 인스턴스가 생성되어 매번 Stack 에 **추가**된다.  
...
- Task 재개
  - 사용자가 나중에 다시 앱아이콘을 선택하는 경우
    ...
    - stack 상단의 `Activity`는 Task 가 background 로 들어가기 전 상태 그대로 다시 시작된다.
...
- 각 인스턴스는 자체 상태와 Back Stack 위치를 사용하여 **독립적**으로 동작한다.
